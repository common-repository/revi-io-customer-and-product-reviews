...
class revimodel
{
    ...
    */
    public function getOrder($id_order)
    {
        ...
        if ($table_exists) {
            ...
            $sql_new = "SELECT O.ID as id_order, O.date_created_gmt as order_date
        ...
        $sql_old = "SELECT O.ID as id_order, O.post_date_gmt as order_date
    ...
    */
    public function getOrders()
    {
        ...
        if ($table_exists) {
            ...
            // Si encontramos pedidos en las nuevas tablas que no estén en revi_orders, los retornamos
            if ($orders) {
                return $orders;
            }
        }

        // Si no hay datos nuevos en las nuevas tablas o no existen, usar las tablas anteriores de WordPress
        $sql_old = "SELECT O.ID as id_order, O.post_date_gmt as order_date
                FROM {$this->prefix}posts O 
    ...
    */
            ...
            foreach ($wc_order->get_items() as $item_id => $item) {
                ...
                if ($product->is_type('variation')) {
                    ...
                    if (version_compare($wc_version, '3.0', '<')) {
                        ...
                        $order_product['id_shop_product_combination'] = $product->get_id();
                    }
                }
    ...
    public function getProduct($id_product, $id_lang)
    {
        ...
        $sql = "SELECT P.id_product, PL.name, P.price, P.ean13 as ean, PL.link_rewrite, P.quantity as stock
    ...
    public function getNumProductsNotSent()
    {
        // Ajuste para permitir consultas grandes, aunque considera evaluar si es necesario tras la optimización
        $this->wpdb->query('SET SQL_BIG_SELECTS = 1');

        // Utilizamos LEFT JOIN para una consulta más eficiente
        $sql = "SELECT COUNT(P.ID) as num_products 
                FROM " . $this->prefix . "posts P 
                LEFT JOIN revi_products RP ON P.ID = RP.id_product 
                WHERE RP.id_product IS NULL 
                AND P.post_type = 'product'";
