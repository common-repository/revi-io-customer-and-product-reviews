...
function revi_save_content()
{
    ...
    global $wpdb;
    $tables_to_check = ['revi_products', 'revi_orders', 'revi_comments'];

    foreach ($tables_to_check as $table_name) {
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
            // Si la tabla no existe, eliminar y recrear la base de datos
            revi_deleteReviDatabase();
            revi_createReviDatabase();
            break; // Salir del bucle al encontrar una tabla que falta
        }
    }
